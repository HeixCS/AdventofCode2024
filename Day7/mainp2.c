...
long long unsigned int concat(long long unsigned int x, long long unsigned int y){
    long long unsigned int pow = 10;
    while(pow < y){
        pow *= 10;
    }
    return x * pow + y;
...
int main(void){
    ...
    while(fgets(buffer,1000,fp)){
        ...
        // Finds the length of the result
        ...
        // Copying in the second half of the string after : 
        ...
        while(not_done){
            ...
            if(opp_order[0] == '*'){
                ...
                pos_result = concat(array[0],array[1]);
            }
            else{
                pos_result = array[0] + array[1];
            }
            ...
            for(int i = 1; i < arr_size - 1; i++){
                if(opp_order[i] == '*'){
                    ...
                    pos_result = concat(pos_result,array[i+1]);
                }
                else{
                    pos_result = pos_result + array[i+1];
                }
