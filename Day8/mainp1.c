    ...
    while(current !=  NULL){
        for(int i = 0; i < current->occurences; i++){
            for(int j = i + 1; j < current->occurences; j++){
                ...
                if(valid_coord(current->list[j].x + run, read_map.x_size) && valid_coord(current->list[j].y + rise, read_map.y_size)){
                    if(anti_map[current->list[j].y + rise][current->list[j].x + run] == '.'){
                        anti_map[current->list[j].y + rise][current->list[j].x + run] = '#';
                        occurences++;
                    }
                    if(read_map.char_map[current->list[j].y + rise][current->list[j].x + run] == '.'){
                        read_map.char_map[current->list[j].y + rise][current->list[j].x + run] = '#';
                    }
                }
                if(valid_coord(current->list[i].x - run, read_map.x_size) && valid_coord(current->list[i].y - rise, read_map.y_size)){
                    if(anti_map[current->list[i].y - rise][current->list[i].x - run] == '.'){
                        anti_map[current->list[i].y -rise][current->list[i].x - run] = '#';
                        occurences++;
                    }
                    if(read_map.char_map[current->list[i].y - rise][current->list[i].x - run] == '.'){
                        read_map.char_map[current->list[i].y -rise][current->list[i].x - run] = '#';
                    }
                }
            }
        }
