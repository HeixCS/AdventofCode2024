...
int main(void){
    ...
    int rule_length = find_rule_length(file_name),array_elements = 0, temp_swap = 0;
    ...
    while(fgets(buffer,1000,fp)){
            ...
            for(int i = 0; i < rule_length;i++){
                ...
                for(int i = 0; i < array_elements;i++){
                    ...
                    if(current->data == second_element){
                        ...
                        while(current != NULL){
                            // Finds first element in pair
                            if(current->data == first_element){
                                second_error = current;
                                printf("Elements to swap %d and %d\n",first_element,second_element);
                                // Swapping up the array
                                while(first_error->next != second_error->next){
                                    temp_swap = first_error->data;
                                    first_error->data = first_error->next->data;
                                    first_error->next->data = temp_swap;
        ...
        // Sees if the string is invalid
        if(!valid){
            // Calculates middle index and finds its value then adds it
